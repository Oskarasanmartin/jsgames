...
function pageReady(){
		...
		// Setup highscore board	
		clearHighscore(storageId);
		highscoreContainer = document.getElementById("highscoreContainer");
		loadHighscore(storageId);
		console.log(highscoreList);
		showHighscore(highscoreContainer);
...
function animateBall(){	
	if(ball.offsetTop > stageBottom){
		...
		// Remove a life
		document.getElementById("life" + lifeCounter).style.opacity = 0.2;
		...
		if(lifeCounter == 0){
			...
			if(score > 0){
				var playerName = prompt("Save your score? Then gimme your name!");
				if(playerName){
					saveHighscore(playerName, storageId, score);
					showHighscore(highscoreContainer);
				}
			}
		}
